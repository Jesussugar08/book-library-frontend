...
import { BookIcon, OpenBookIcon, StarIcon } from '../components/icons/AppIcons'
...
function StatMainCard({ icon, value, label, color }) {
  return (
    <Box bg="surface.bg" borderRadius="10px" border="1px" borderColor="surface.border" p={4}>
      <Box mb={2} color="text.secondary">{icon}</Box>
      ...
      {loading ? (
        ...
        <>
          ...
          <SimpleGrid columns={{ base: 1, sm: 2 }} spacing={2.5} mb={4}>
            ...
            <StatMainCard icon={<BookIcon boxSize={5} />} value={stats.total} label="Total books" color="#2B6CB0" />
            <StatMainCard icon={<StarIcon boxSize={5} color="#F6AD55" />} value={avgRating} label="Avg rating" color="#744210" />
            <StatMainCard icon={<OpenBookIcon boxSize={5} />} value={stats.leyendo} label="Reading now" color="#553C9A" />
            <StatMainCard icon={<CheckCircleIcon boxSize={5} />} value={stats.leidos} label="Read" color="#276749" />
