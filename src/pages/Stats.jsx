...
function StatMainCard({ icon, value, label, color }) {
  return (
    <Box bg="white" borderRadius="10px" border="1px" borderColor="#E2E8F0" p={4}>
      <Text fontSize="22px" mb={2}>{icon}</Text>
      <Text fontSize="28px" fontWeight="600" color={color}>{value}</Text>
      <Text fontSize="12px" color="#718096" mt="2px">{label}</Text>
    </Box>
  )
}
...
function BarRow({ label, count, max, color }) {
  ...
  return (
    <Box mb={2.5}>
      <Flex justify="space-between" fontSize="12px" color="#4A5568" mb={1.5}>
        <Text>{label}</Text>
        <Text>{count}</Text>
      </Flex>
      <Box bg="#EDF2F7" borderRadius="4px" h="8px" overflow="hidden">
    ...
    <AppLayout maxW="700px" navTitle="My statistics" backTo="/dashboard">
      ...
      {loading ? (
        ...
        <>
          ...
          <Box bg="white" borderRadius="10px" border="1px" borderColor="#E2E8F0" p={4} mb={3.5}>
            <Text fontSize="13px" fontWeight="600" color="#1A202C" mb={3.5}>
              Books by status
            </Text>
          ...
          <Box bg="white" borderRadius="10px" border="1px" borderColor="#E2E8F0" p={4}>
            <Text fontSize="13px" fontWeight="600" color="#1A202C" mb={3}>
              Top rated books
            </Text>
            {topRated.length === 0 ? (
              <Text fontSize="13px" color="#718096">No rated books yet.</Text>
            ) : (
              topRated.map((book, i) => (
                <Flex
                  key={book.id}
                  ...
                  borderColor="#F7FAFC"
                >
                  <Text fontSize="13px" fontWeight="600" color="#A0AEC0" w="18px">
                    {i + 1}
                  </Text>
                  <Box flex={1}>
                    <Text fontSize="13px" fontWeight="500" color="#1A202C">{book.title}</Text>
                    <Text fontSize="11px" color="#718096">{book.author}</Text>
                  </Box>
